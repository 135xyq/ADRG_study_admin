          ...
          <template slot-scope="scope">
            ...
            <el-row>
              <el-button
                ...
                size="mini"
                type="success"
                @click="onHandleGoToQuestion(scope.row.id)"
              >题目列表
              </el-button>
              <el-button icon="el-icon-s-release" size="mini" type="info" @click="onHandleGoToQuestionReslove(scope.row.id)">
                刷题记录
              </el-button>
            </el-row>
          </template>
...
<script>
// 题目的分类
import { deleteCategory, page, update } from '@/api/questionCategory'
...
export default {
  ...
  methods: {
    ...
    async onHandleDelete(id) {
      this.$confirm('此操作将删除该分类，并删除该分类下的题目, 是否继续?', '提示', {
